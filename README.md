<!--
  SEO META TAGS — Indexed by search engines crawling GitHub profiles
  Name: Rivindu Ashinsa
  Keywords: Rivindu Ashinsa, AI Engineer, Machine Learning, Healthcare AI, CognivusLabs, Sri Lanka
  Description: Rivindu Ashinsa is an AI Research & Development Engineer specializing in biomedical machine learning, ECG analysis, and agentic AI systems.
  Author: Rivindu Ashinsa
  Canonical: https://github.com/rivindu-ashinsa
  og:title: Rivindu Ashinsa — AI R&D Engineer
  og:description: Rivindu Ashinsa builds production-grade AI systems for healthcare — from ECG abnormality detection to SpO₂ prediction and agentic frameworks.
  og:url: https://github.com/rivindu-ashinsa
  twitter:card: summary
  twitter:title: Rivindu Ashinsa
  twitter:description: AI R&D Engineer | Machine Learning | Biomedical AI | CognivusLabs
-->

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=160&section=header&text=Rivindu%20Ashinsa&fontSize=42&fontColor=ffffff&fontAlignY=38&desc=AI%20R%26D%20Engineer%20%7C%20Biomedical%20ML%20%7C%20Agentic%20AI&descSize=16&descAlignY=58&descColor=a78bfa" alt="Rivindu Ashinsa — AI R&D Engineer" />

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Rivindu%20Ashinsa-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rivindu-ashinsa/)
[![Portfolio](https://img.shields.io/badge/Portfolio-rivindu--ashinsa.github.io-7C3AED?style=for-the-badge&logo=google-chrome&logoColor=white)](https://rivindu-ashinsa.github.io)
[![Email](https://img.shields.io/badge/Email-ashinsa.rivindu%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ashinsa.rivindu@gmail.com)
[![Instagram](https://img.shields.io/badge/Instagram-@__rvndq__-E1306C?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/__rvndq__)

<br/>

![Profile Views](https://komarev.com/ghpvc/?username=rivindu-ashinsa&style=flat-square&color=7C3AED&label=Profile+Views)

</div>

---

## `$ whoami`

```python
rivindu_ashinsa = {
    "role"      : "AI Research & Development Engineer",
    "company"   : "CognivusLabs",
    "location"  : "Sri Lanka 🇱🇰",
    "domains"   : ["Biomedical ML", "Agentic AI", "Time-Series Analysis"],
    "building"  : "AI that bridges research and real-world healthcare impact",
    "philosophy": "Data is the new medicine; AI is the new lab."
}
```

I build **production-grade ML systems** for real-time biomedical applications — from ECG arrhythmia detection to SpO₂ forecasting. My work sits at the intersection of rigorous research and engineering reliability.

**What drives me:**

- 🏥 Applying AI to healthcare through predictive modeling and signal processing
- 🧩 Designing interpretable, production-aware systems from research prototypes
- 🤖 Architecting agentic AI frameworks capable of reasoning and autonomous execution
- ⚡ Pushing efficient AI to edge devices for real-time health monitoring

---

## 🧰 Technical Stack

<table>
<tr>
<td valign="top" width="50%">

**Core ML & Data Science**
```
Python · SQL · NumPy · pandas · Polars
scikit-learn · PyTorch · TensorFlow
Seaborn · Matplotlib · Plotly
```

**Deep Learning**
```
CNNs · RNNs · Transformers
Time-Series Forecasting
Computer Vision (OpenCV) · NLP
```

</td>
<td valign="top" width="50%">

**Agentic AI & LLMs**
```
LangGraph · LangChain · Agno
Multi-Agent Systems · ReAct · RAG
Prompt Engineering · Tool Calling
```

**MLOps & Engineering**
```
Git · GitHub · Docker
CI/CD Pipelines · Model Serving
API Development · Edge Deployment
```

</td>
</tr>
</table>

> **Domain Specialization:** Healthcare & Biomedical AI — ECG processing · SpO₂ prediction · Vital signs analysis · Clinical decision support

---

## 🔬 Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### 🏥 CognivusMed — Biomedical AI Suite
*AI R&D Engineer · CognivusLabs*

Real-time biomedical signal intelligence platform.

- **ECG Abnormality Detection** — ML models for arrhythmia classification and waveform anomaly detection
- **SpO₂ Prediction Engine** — 180-second oxygen saturation forecasting from live signal streams
- **Impact** — High-accuracy, low-latency predictions enhancing patient monitoring

</td>
<td width="50%" valign="top">

### 🧠 Dementia Prediction System
*Team-XPredators*

Early-stage dementia risk assessment using longitudinal clinical data.

- Robust ML models on behavioral and clinical inputs
- Feature engineering from long-horizon patient records
- Focus on interpretable outputs for clinical usability

</td>
</tr>
<tr>
<td width="50%" valign="top">

### ❤️ Heart Disease Prediction Pipeline
*Healthcare Analytics*

End-to-end cardiovascular risk assessment system.

- Advanced preprocessing & ensemble modeling
- Interpretable outputs with Explainable AI techniques
- Deployable clinical decision support module

</td>
<td width="50%" valign="top">

### 🤖 RepoDigest
*Open-Source Tool*

Intelligent GitHub repository analysis and summarization.

- Extracts structural insights and documentation patterns
- Auto-generates comprehensive project summaries
- Useful for onboarding and open-source discovery

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 📦 SupplyGenie
*OutllierRejects × Idealize*

ML-driven supply chain optimization platform.

- Demand forecasting and inventory intelligence
- Decision support for logistics workflows
- End-to-end ML pipeline with production deployment

</td>
<td width="50%" valign="top">

### ⚡ EdgeVitalsAI
*Edge Computing for Healthcare*

Lightweight vital-sign AI for edge hardware.

- Efficient model architectures for constrained devices
- Real-time health monitoring with minimal overhead
- Contributions to open-source edge health tooling

</td>
</tr>
</table>

---

## 📊 GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=rivindu-ashinsa&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=a78bfa&icon_color=7C3AED&text_color=c9d1d9" alt="Rivindu Ashinsa GitHub Stats"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=rivindu-ashinsa&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=a78bfa&text_color=c9d1d9" alt="Rivindu Ashinsa Top Languages"/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=rivindu-ashinsa&theme=tokyonight&hide_border=true&background=0D1117&ring=7C3AED&fire=a78bfa&currStreakLabel=a78bfa" alt="Rivindu Ashinsa GitHub Streak"/>

</div>

---

## 💡 Research Interests

| Area | Focus |
|------|-------|
| 🏥 **Healthcare & Biomedical AI** | ECG, SpO₂, predictive patient monitoring |
| 🤖 **Agentic AI & Multi-Agent Systems** | Autonomous reasoning, planning, tool use |
| 📊 **Time-Series & Anomaly Detection** | Biomedical signal processing, forecasting |
| 🔍 **Explainable AI** | Interpretability for clinical systems |
| ⚡ **Edge AI** | Efficient inference on resource-constrained devices |
| 🧠 **LLMs in Healthcare** | Clinical NLP, decision support, RAG pipelines |
| 🔐 **Federated Learning** | Privacy-preserving ML for medical data |

---

## 🎓 Professional Background

**AI & ML Engineering · CognivusLabs**
Developing and deploying predictive models for biomedical applications. Building validated data pipelines and production-grade AI solutions in cross-functional research teams.

**Open-Source Leadership**

| Organization | Focus |
|---|---|
| [Team-XPredators](https://github.com/Team-XPredators) | Healthcare AI Research |
| [OutllierRejects](https://github.com/OutllierRejects) | Supply Chain & Logistics ML |
| [EdgeVitalsAI](https://github.com/EdgeVitalsAI) | Edge Computing for Health Monitoring |

**Academic Foundation**
Computer Science undergraduate — integrating theoretical ML foundations with applied research across biomedical, agentic, and edge AI domains.

---

## 🤝 Let's Connect

I'm open to:

- 🔬 **Research Collaborations** — ML in healthcare and biomedical domains
- 🌟 **Open-Source Projects** — AI/ML tools, health-tech, agentic frameworks
- 🎤 **Speaking / Workshops** — AI/data science events and tech meetups
- 👨‍🏫 **Mentorship** — Guiding early-stage ML engineers and researchers

<div align="center">

| 📧 Email | 💼 LinkedIn | 📸 Instagram |
|---|---|---|
| [ashinsa.rivindu@gmail.com](mailto:ashinsa.rivindu@gmail.com) | [rivindu-ashinsa](https://www.linkedin.com/in/rivindu-ashinsa/) | [@__rvndq__](https://www.instagram.com/__rvndq__) |

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=100&section=footer" alt="footer"/>

*"Data is the new medicine; AI is the new lab."*

**Rivindu Ashinsa** · AI R&D Engineer · Building intelligent systems that save lives, one algorithm at a time.

`rivindu ashinsa` · `rivindu-ashinsa` · `ashinsa rivindu` · AI Engineer · CognivusLabs · Sri Lanka

</div>
